...
const lines: LineItem[] = [
  ...
  { text: "This document outlines the architecture for an autonomous, AI-driven web3 market intelligence platform. Leveraging LLMs, real-time data aggregation, Solana, and Eliza OS, the platform delivers powerful market insights and executes strategic investments autonomously all while engaging users 24/7 on social media.", isHeading: false },
  { text: "", isHeading: false },
  { text: "## 2. System Overview & Architectural Components", isHeading: true },
  { text: "Key highlights include:", isHeading: false },
  { text: "- A hierarchical brain that synthesises insights from specialised sub-agents.", isHeading: false },
  { text: "- An aggregation layer that normalises and validates real-time data from diverse API sources such as CoinGecko, LunarCrush, Certik, and DefiLlama.", isHeading: false },
  { text: "- A dynamic conversational interface powered by Eliza OS, ensuring seamless 24/7 engagement across platforms like X, Telegram, and Discord.", isHeading: false },
  ...
  { text: "- A scalable, cloud-native infrastructure designed for auto-scaling and future expansion.", isHeading: false },
  { text: "", isHeading: false },
  { text: "## 3. Cognitive Architecture", isHeading: true },
  { text: "The cognitive architecture is structured across five layers (Frontal Lobe, Parietal Lobe, Temporal Lobe, Brain STEM, Qualia Layer), each contributing to a comprehensive analysis and decision-making process. View the cognitive hierarchy here. A description of each is below:", isHeading: false },
  { text: "", isHeading: false },
  ...
  { text: "- Triggers validation checks to ensure data integrity (e.g. date formats, timestamps, currency units).", isHeading: false },
  { text: "- Flags missing or anomalous data for secondary validation.", isHeading: false },
  { text: "", isHeading: false },
  { text: "### 3.2. Brain STEM", isHeading: true },
  { text: "- The most granular level of LLMs analyses and summarises API data (eg social metrics) and information (eg. Litepaper). Each LLM in this layer is highly specialised.", isHeading: false },
  { text: "- Provides analysis with scores and qualitative assessment to domain-specific Temporal Lobe LLMs.", isHeading: false },
  { text: "", isHeading: false },
];

// Render each line based on its content.
function renderLineItem(line: string, key: number) {
  const trimmedLine = line.trimStart();

  // Check for subheadings (###) and headings (##) using simple string methods.
  ...
  // Other formatting rules (asterisks, dashes, etc.)
  if (trimmedLine.startsWith("* ")) {
    return (
...
export default function LitepaperContent() {
  ...
  // When the current line is a heading, immediately add it.
  useEffect(() => {
    if (currentIndex < lines.length && lines[currentIndex].isHeading) {
      setCompletedItems((prev) => [...prev, lines[currentIndex]]);
      setCurrentIndex((prev) => prev + 1);
    }
  }, [currentIndex]);

  // When a non-heading line is complete (via the typewriter), add it.
  ...
  return (
    <div className="space-y-2 font-sf-pro text-sm text-gray-600">
      ...
      {currentIndex < lines.length &&
        // Only animate non-heading lines.
        !lines[currentIndex].isHeading && (
          <TypewriterText
            key={currentIndex}

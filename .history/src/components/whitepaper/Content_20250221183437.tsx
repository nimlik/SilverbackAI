...
function renderLine(line: string, key: number) {
  // Use regex for more flexible heading detection
  const headingMatch = line.trimStart().match(/^(#{2,3})\s+(.*)/);
  ...
  // Preserve original formatting rules with exact matches
...
export default function LitepaperContent() {
  ...
  // Keep lines array EXACTLY as provided
  const lines = [
   "## Table of Contents",
    "1. Introduction",
  ...
  const handleLineComplete = () => {
    if (currentLine < lines.length) {
      const nextLine = lines[currentLine].trim(); // Trim to remove unwanted spaces
      setCompletedLines(prev => [...prev, nextLine]);

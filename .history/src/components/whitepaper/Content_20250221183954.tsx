...
export default function LitepaperContent() {
  ...
  // The full list of lines (preserve exact formatting)
  const lines = [
    "## Table of Contents",
    "1. Introduction",
  ...
  // Instead of waiting for the typewriter effect, if the current line is a heading we add it immediately.
  ...
  // Auto-complete heading lines as soon as they're encountered.
  useEffect(() => {
    if (currentLine < lines.length) {
      const currentText = lines[currentLine].trimStart();
      if (currentText.startsWith("##") || currentText.startsWith("###")) {
        handleLineComplete();
      }
    }
    // We want this to run whenever currentLine changes.
  }, [currentLine]);
  ...
  return (
    <div className="space-y-2 font-sf-pro text-sm text-gray-600">
      ...
      {currentLine < lines.length &&
        // Only use the typewriter effect if the line is not a heading
        !lines[currentLine].trimStart().match(/^(#{2,3})\s+(.*)/) && (
          <TypewriterText
            key={currentLine}

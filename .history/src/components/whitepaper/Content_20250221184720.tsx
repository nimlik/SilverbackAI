...
function renderLine(line: string, key: number) {
  ...
  // Other formatting rules remain unchanged
  if (trimmedLine.startsWith("* ")) {
    return (
...
export default function LitepaperContent() {
  ...
  // The array of lines (preserve exact formatting)
  const lines = [
    "## Table of Contents",
    "1. Introduction",
  ...
  // Auto-advance over heading lines.
  const advanceHeadingLines = () => {
    let nextIndex = currentLine;
    const newCompleted: string[] = [];
    while (nextIndex < lines.length && isHeading(lines[nextIndex])) {
      newCompleted.push(lines[nextIndex]);
      nextIndex++;
    }
  ...
  // On currentLine change, if it's a heading, auto-advance it.
  useEffect(() => {
    if (currentLine < lines.length && isHeading(lines[currentLine])) {
      advanceHeadingLines();
    }
  }, [currentLine]);

  // When the typewriter animation completes a non-heading line.
